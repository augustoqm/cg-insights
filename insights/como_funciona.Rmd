...
datatable(vereadores_partidos, options = list(paging = F, searching = F, info = F), rownames = F)
```

### Mesa Diretora
```{r, echo=FALSE}
mesa_diretora <- read_csv("../docs/legislatura_16_mesa_diretora.csv")
datatable(mesa_diretora, options = list(paging = F, searching = F, info = F, ordering = F), rownames = F)
```

### Comissões Técnicas
```{r, echo=FALSE}
comissoes <- read_csv("../docs/legislatura_16_comissoes_tecnicas.csv")
datatable(comissoes, options = list(paging = F, searching = F, info = F), rownames = F)
```
